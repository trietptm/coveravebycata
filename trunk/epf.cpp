...
void toggle_tracer(void)
{
	...
	if( false == b_switch )
	{
		...
		{
		case 0:
			return_value = get_segm_name(get_reg_val("eip"), (char *)(aub_DummyBuffer), MAXSTR);
			if( return_value != (ssize_t)(-1) )
			{
				//no error
				qstrncpy(seg,aub_DummyBuffer,MAXSTR-1);
			}
			else
			{
				//error !
				msg("-> EPF: Error in get_segm_name");
			}
...
/*		case 2:
	...
	if( b_switch ) 
	{
		msg ("-> EPF: Please resume the process now!\n");
	}
}
//-------------------------------------------------------------
...
static int idaapi dbg_callback(void * /*user_data*/, int event_id, va_list /*va*/)
{
	...
	if(event_id==dbg_trace)
	{
		...
		{
		case 0:
			get_segm_name(get_reg_val("eip"),(char *)(aub_DummyBuffer), MAXSTR);
			if( strcmp(seg, aub_DummyBuffer) != 0 )
			{
				msg("-> EPF: EIP is pointing into a different section at: %08X.\n",eip);			
				//set_process_state(dbg_suspend_process, NULL, DBGINV_NONE);
...
void idaapi run(int arg)
{
	...
	if( false == start_process() )
	{
		msg("--> error: start_process\n");
	}
	
	

	if( false == add_bpt(0x00406374) )
	{
		msg("--> error: add_bpt\n");
	}

	
	if( false == request_continue_process() )
	{
		msg("--> error: continue_process\n");
	}
...
char wanted_name[] = "Toggle EPF tracer on or off Cata___";
